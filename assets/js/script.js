...
// ---------- Testimonials (single consolidated implementation) ----------
(function($){
  $(function(){

    // keep handler bound BEFORE init so 'init' fires to set the active thumbnail
    $('.testimonials .slider-content').on('init afterChange', function(event, slick, currentSlide){
      var idx = (typeof currentSlide === 'number') ? currentSlide : slick.currentSlide;
      // remove active from all thumbnail nodes
      $('.testimonials .slider-nav .slider-nav-img').removeClass('active');
      // add active to thumbnail that has the matching data-index
      $('.testimonials .slider-nav .slider-nav-img[data-index="'+idx+'"]').addClass('active');
    });
    ...
    // click on thumbnail DOM elements (some devs keep this; slick's focusOnSelect also handles navigation
    // but this ensures clicking the original thumbnail (not the cloned ones) will move slider.
    $('.testimonials').on('click', '.slider-nav-img', function(){
      var i = $(this).data('index');
      if (typeof i !== 'undefined') {
        $('.testimonials .slider-content').slick('slickGoTo', parseInt(i));
      }
    });
